...
def attribution_metrics(
    ...
    **kwargs) -> pd.DataFrame:
    ...
    - method='wwa' (default): delegates to
      `climattr.attribution_wwa.attribution_metrics(all, covariates_df,
    ...
    - method='shift': delegates to
      `climattr.attribution.attribution_metrics(all, nat, fit_function,
      thresh, direction='descending', bootstrap_ci=95, boot_size=1000,
      summary_statistics=True, n_jobs=-1)`. `all`/`nat` must already be
      WWA-shifted series (e.g. via `climattr.minimization.fit.shift_data`
      / `get_wwa_series`) and `fit_function` a scipy.stats distribution
      object (not a name string). Bootstraps by resampling each shifted
      series independently and refitting a stationary distribution to it.
      Returns PR, FAR (Fraction of Attributable Risk) and return periods
      for the ALL/NAT climates - the classic WWA event-shifting approach.
    ...
    **kwargs
    ...
    """
    if method == 'wwa':
        return attribution_wwa.attribution_metrics(**kwargs)

    if method == 'shift':
        return attribution.attribution_metrics(**kwargs)
